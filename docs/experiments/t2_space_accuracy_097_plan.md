...
6. batch size 8は、固定した最大8件の学習・検証一覧と新規出力先を必須にした1 step相当のWSL試験から始める。この試験値は精度判定に使わない。
...
3. 固定小規模一覧でbatch size 8のGPU試験を実行する。
4. 監査結果から、方向変更とAPTA全面置換は候補外とする。学習側だけのデータ拡張または脊柱管を重視する損失調整から変更点を1つ選び、新しいキャンペーンとして学習する。
5. 固定検証一覧を3方式で評価し、3クラスすべてと前景平均をbaselineと比較する。
